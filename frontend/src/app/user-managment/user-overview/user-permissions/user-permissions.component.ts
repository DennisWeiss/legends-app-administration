...
export class UserPermissionsComponent implements OnInit {
  ...
  perms: string[] = ['EDIT'];
  allPerms: string[] = ['ADMIN', 'EDIT', 'EDIT_CONTENT'];
  ...
  add(event: MatChipInputEvent): void {
    ...
    if (!this.matAutocomplete.isOpen) {
      ...
      // Add permission
      if ((value || '').trim() && !this.user.permissions.includes(value) && this.allPerms.includes(value)) {
  ...
  remove(fruit: string): void {
    const index = this.user.permissions.indexOf(fruit);
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    ...
    if (!this.user.permissions.includes(value)) {
